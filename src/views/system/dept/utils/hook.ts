...
export function useDept() {
  ...
  function openDialog(title = '新增', row?: FormItemProps) {
    addDialog({
      ...
      props: {
        formInline: {
          higherDeptOptions: formatHigherDeptOptions(deepClone(dataList.value)),
      ...
      beforeSure: (done, { options }) => {
        ...
        function chores() {
          ElMessage.success(`您${title}了部门名称为${curData.name}的这条数据`)
          done() // 关闭弹框
          onSearch() // 刷新表格数据
        }
  ...
  function handleDelete(row) {
    ElMessage.success(`您删除了部门名称为${row.name}的这条数据`)
    onSearch()
  }

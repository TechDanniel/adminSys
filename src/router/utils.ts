...
function filterNoPermissionTree(data:RouteComponent[]){
    const newTree=deepClone(data).filter((v:any)=>{
    ...
    if (routeList.length )  {
        ...
        formatFlatteningRoutes(addAsyncRoutes(routeList)).map(
            (v)=>{
                ...
                if(router.options.routes[0].children.findIndex(value=>{value.path===v.path})!==-1) return
                else{
...
function getTopMenu(tag = false): menuType {
    ...
    const topMenu = handleTopMenu(
      usePermissionStore().wholeMenu[0]?.children[0]
    );
...
function hasAuths(value:string|Array<string>):boolean{
    ...
    //判断是否有权限
    const isAuths=isString(value)?metaAuths.includes(value):
        isIncludeAllChildren(metaAuths,value)
    return isAuths
}   

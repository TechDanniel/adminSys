...
formaTwoStageRoutes(formatFlatteningRoutes(ascending(routes.flat(Infinity))))
...
async function handleDynamicRoutesAndTags(to,from, next) {
    if(usePermissionStore().wholeMenu.length===0&&to.path!=="login"){   
...
router.afterEach(() => {
    console.log("完整路由信息",router.options.routes)
    Nprogress.done();
});

...
interface DialogOptions extends DialogProps {
  ...
  footerRender?: ({ options, index }: { options: DialogOptions; index: number }) => VNode | Component
  /** 自定义底部按钮操作 */
  footerButtons?: Array<ButtonProps>
  /**Dialog打开后的回调 */
  open?: ({ options, index }: { options: DialogOptions; index: number }) => void
  /**Dialog关闭后的回调（只有点击右上角关闭按钮或空白页或按下了esc键关闭页面时才会触发） */
...
export type { DialogOptions, DialogProps, BtnClickDialog, BtnClickButton, ButtonProps, EventType }

                ...
                <div v-for="(item,index) in multiTags"
                    ...
                    :class="[
                       'scroll-item is-closable',
                        linkIsActive(item),
                        isFixedTag(item,index) && 'fixed-tag' 
                    ]"
...
const dynamicTagView=async()=>{
    ...
    const index=multiTags.value.findIndex(item=>{
        if(!isEmpty(route.query)){
            return _.isEqual(item.query,route.query)
        }else if(!isEmpty(route.params)){
            return _.isEqual(item.params,route.params)
        }else{

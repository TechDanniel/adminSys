...
class Http{
                    ...
                    if(data){
                        ...
                        if(expired){
                            if(!Http.isRefreshing){
                                ...
                                //刷新过期的token
                                useUserStore().handleRefreshToken({refreshToken:data.refreshToken}).then(
                                    res=>{
                                        //得到新的token
    ...
    /**发送请求的工具函数 */
    public request<T>(method:RequestMethods,url:string,params?:AxiosRequestConfig,axiosConfig?:HttpRequestConfig):Promise<T>{
        return new Promise((resolve,reject)=>{
            Http.axiosInstance.request({method,url,...params,...axiosConfig}).then(
                (response)=>{resolve(response as unknown as T)}

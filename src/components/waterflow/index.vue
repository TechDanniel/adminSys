...
const renderState=reactive({
    queue:Array(props.column).fill(0).map<columnQueue>(()=>({list:[],height:0})),
...
const reComputedQueue=()=>{
    //清空renderState所有卡片
    renderState.queue=Array(props.column).fill(0).map<columnQueue>(()=>({list:[],height:0}))
    renderState.len=0
    setItemSize()
